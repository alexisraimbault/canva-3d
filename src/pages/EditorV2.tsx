...
export const EditorV2 = ({}: EditorV2PropsTypes) => {
  ...
  const [projectData, setProjectData] = useState<ProjectV2Type>({
    ...defaultProject,
  });
  const [selectedItemIndexPath, setSelectedItemIndexPath] = useState<
    number[] | null
  >(null);
  ...
  const isSidebarBackgroundSelection =
    selectedItemIndexPath !== null &&
    selectedItemIndexPath.length === 1 &&
    selectedItemIndexPath[0] === -1;
  const isSidebarItemSelected =
    selectedItemIndexPath !== null && !isSidebarBackgroundSelection;
  ...
  const fetchInitialData = async () => {
    ...
    const projectRef = ref(
      database,
      `users/${userId}/projects/${urlProjectId}`
    );
  ...
  const onPublish = async () => {
    ...
    const allNames =
      snapshotData === null
        ? []
        : Object.keys(snapshotData).map((item) => item.toLowerCase());
    ...
    const projectPublishNameRef = ref(
      database,
      `publishNames/${projectData.name}`
    );
    const dataToWrite = {
      projectId,
      userId,
  ...
  const setItemPathValueAndRetrieveRec = (
    path: number[],
    newItemValue: ItemType,
    itemsTree: ItemType[]
  ) => {
    if (path.length === 1) {
  ...
  const removeItemWithPathAndRetrieveRec = (
    path: number[],
    itemsTree: ItemType[]
  ) => {
    if (path.length === 1) {
    ...
    itemsTree[path[0]].containerData!.children =
      removeItemWithPathAndRetrieveRec(
        path.slice(1),
        itemsTree[path[0]]?.containerData?.children || []
      );
  ...
  const editItem = (itemIndexPath: number[] | null, newItemValue: ItemType) => {
    ...
    const newProject = { ...projectData };
    newProject.items = setItemPathValueAndRetrieveRec(
      itemIndexPath,
      newItemValue,
      newProject.items
    );
  ...
  const deleteItem = (itemIndexPath: number[] | null) => {
    ...
    const newProject = { ...projectData };
    newProject.items = removeItemWithPathAndRetrieveRec(
      itemIndexPath,
      newProject.items
    );
    setProjectData(newProject);
  ...
  const hasNoSpecialBackground =
    (projectData?.globalBgSpecialSettings?.type || "none") === "none";
  ...
  return (
    <div className="editorv2__wrapper">
      ...
      <div className="editorv2__inner">
        ...
        />
        <div
          className={`editorv2__page-content
                            ${
                              isSidebarBackgroundSelection ||
                              isSidebarItemSelected
                                ? " editorv2__page-content--reduced"
                                : ""
                            }
                            `}
            ...
            {projectData?.items?.map((projectItem, projectItemIndex) => {
              return (
                <EditorV2ItemRenderer
                  ...
                  isSelected={
                    selectedItemIndexPath?.length === 1 &&
                    selectedItemIndexPath[0] === projectItemIndex
                  }
                  onSelectItem={() =>
                    setSelectedItemIndexPath([projectItemIndex])
                  }
                  setSelectedItemIndexPath={setSelectedItemIndexPath}
                  relativeSelectedItemIndexPath={selectedItemIndexPath || []}
                  isAncestorsIndexPathMatching={
                    (selectedItemIndexPath?.length &&
                      selectedItemIndexPath?.length >= 1 &&
                      selectedItemIndexPath[0] === projectItemIndex) ||
                    false
                  }
